<?php
	...
	//$conexao->exec($drop); 
	$create = "create table if not exists consulta (id integer primary key autoincrement, cpf_pac text, nome_pac text, data_cons date, descricao text, status text, fechamento text, clinico text, encaminhamento text, especialista text, data_esp text); ";
	$conexao->exec($create);
	...
	if ( $resultado1 > 0 ) {
		$conexao = new pdo('sqlite:bancodedados.data');
		$pesquisa7 = "select * from paciente where cpf = '".$_REQUEST['cpf_pac']."'; ";
		$resultado6 = $conexao->query($pesquisa7)->fetchAll();
		if (count($resultado6) > 0) {
			$update2 = "update consulta set nome_pac = '".$resultado6[0]['nome']."' where cpf_pac = '".$_REQUEST['cpf_pac']."';";
